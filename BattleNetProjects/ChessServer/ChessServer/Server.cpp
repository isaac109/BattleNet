...
bool Server::ListenForNewConnection()
{
	...
	else //If client connection properly accepted
	{
		...
		PlayersStates[TotalConnections] = GameState::G_GlobalServer;
		ClientThreads[TotalConnections] = CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)ClientHandlerThread, (LPVOID)(TotalConnections), NULL, NULL); //Create Thread to handle this client. The index in the socket array for this thread is the value (i).
...
bool Server::ProcessPacket(int ID, Packet _packettype)
{
	...
	case P_GameStateChange:
	{
		...
		{
			PlayersStates[ID] = GameState::G_Waiting_For_chess;
			std::cout<< ID<<PlayersStates[ID]<<std::endl;
		}
		break;
	}
	case P_ChessMove: //Packet Type: chat message
	{
		...
		if (!GetString(ID, Message)) //Get the chat message and store it in variable: Message
			return false; //If we do not properly get the chat message, return false
						  //Next we need to send the message out to each user
		std::string oppID = Message.substr(Message.find_last_of(":") + 1);
		int opponentID = std::stoi(oppID);
		if (!SendString(opponentID, Message)) //Send message to connection at index i, if message fails to be sent...
		{
			std::cout << "Failed to send message from client ID: " << ID << " to client ID: " << opponentID << std::endl;
		}
		std::cout << "Processed chat message packet from user ID: " << ID << std::endl;
		break;
	}
...
void Server::StartChessGames()
{
	...
	while(true)
	{
		...
		{
			if(serverptr->PlayersStates[i] == GameState::G_Waiting_For_chess && std::find(waiting.begin(), waiting.end(), i) == waiting.end())
			{
				waiting.push_back(i);
			}
		...
		if(waiting.size() >= 2)
		{
			...
			serverptr->PlayersStates[p1] = GameState::G_In_Chess;
			serverptr->PlayersStates[p2] = GameState::G_In_Chess;
			std::string Message = "InitChessP1|OpponentID:" + std::to_string(p2);
			...
			Message = "InitChessP2|OpponentID:" + std::to_string(p1);
